...
class FoxLiGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Fox-Li LASER Cavity Simulator (v1 - NumPy)")
    ...
    def run_iteration(self):
        ...
        if self.iter >= self.max_iter:
            ...
            self.total_simulation_time = sim_end_time - self.sim_start_time
            print(f"Total time for {self.max_iter} iterations (including GUI rendering & delays): {self.total_simulation_time:.3f} seconds")
            self.simulation_running = False
    ...
    def calculate_far_field(self):
        ...
        self.Dr = Dr
        self.Drho = Drho
        self.Dr_gauss = Dr_gauss
        self.Drho_gauss = Drho_gauss
    ...
    def save_result_text(self, filename):
        if not hasattr(self, "M2"):
            QMessageBox.warning(self, "Far-Field Not Calculated", "Please calculate the Far-Field (M²) before saving the result text.")
            return

        with open(filename, "w") as f:

            f.write("=" * 80 + "\n")
            f.write("FOX-LI LASER CAVITY SIMULATOR\n")
            f.write("\n")
            f.write("Version 1\n")
            f.write("Engine: NumPy\n")
            f.write("=" * 80 + "\n")
            ...
            f.write("-" * 80 + "\n")
            f.write(f"Grid Size (N)                  : {self.N}\n")
            f.write(f"Wavelength (um)                : {self.wav * 1e6:.6f}\n")
            f.write(f"Pixel Size (um)                : {self.p * 1e6:.6f}\n")
            f.write(f"Propagation Distance (m)       : {self.z:.6f}\n")
            f.write(f"Number of Iterations           : {self.iter}\n\n")

            # MIRROR PARAMETERS
            f.write("MIRROR PARAMETERS\n")
            f.write("-" * 80 + "\n")
            f.write(f"Mirror-1 Radius of Curvature (m) : {self.R1:.8f}\n")
            f.write(f"Mirror-2 Radius of Curvature (m) : {self.R2:.8f}\n")
            f.write(f"Mirror-1 Diameter (mm)           : {self.D1 * 1e3:.6f}\n")
            f.write(f"Mirror-2 Diameter (mm)           : {self.D2 * 1e3:.6f}\n")
            f.write(f"Mirror-1 Conic Constant          : {self.k01:.6f}\n")
            f.write(f"Mirror-2 Conic Constant          : {self.k02:.6f}\n\n")

            # MIRROR MISALIGNMENT
            f.write("MIRROR MISALIGNMENT\n")
            f.write("-" * 80 + "\n")

            f.write("Mirror-1\n")
            f.write(f"    X Offset (um)             : {self.x1 * 1e6:.6f}\n")
            f.write(f"    Y Offset (um)             : {self.y1 * 1e6:.6f}\n")
            f.write(f"    X Tilt (urad)             : {self.theta_x1 * 1e6:.6f}\n")
            f.write(f"    Y Tilt (urad)             : {self.theta_y1 * 1e6:.6f}\n\n")

            f.write("Mirror-2\n")
            f.write(f"    X Offset (um)             : {self.x2 * 1e6:.6f}\n")
            f.write(f"    Y Offset (um)             : {self.y2 * 1e6:.6f}\n")
            f.write(f"    X Tilt (urad)             : {self.theta_x2 * 1e6:.6f}\n")
            f.write(f"    Y Tilt (urad)             : {self.theta_y2 * 1e6:.6f}\n\n")

            # GAIN INFORMATION
            f.write("GAIN INFORMATION\n")
            f.write("-" * 80 + "\n")
            f.write(f"Gain Mode                     : {self.gain_combo.currentText()}\n")

            if self.gain_filepath:
                f.write(f"Gain File                     : {self.gain_filepath}\n")
            else:
                f.write("Gain File                     : None\n")
            ...
            f.write("-" * 80 + "\n")
            f.write(f"M² Factor                     : {self.M2:.8f}\n")
            f.write(f"Dr                            : {self.Dr:.8e}\n")
            f.write(f"D_rho                         : {self.Drho:.8e}\n")
            f.write(f"Dr (Gaussian)                 : {self.Dr_gauss:.8e}\n")
            f.write(f"D_rho (Gaussian)              : {self.Drho_gauss:.8e}\n")

            if hasattr(self, "total_simulation_time"):
                f.write(f"Total Simulation Time (s)      : {self.total_simulation_time:.3f}\n")

                if self.iter > 0:
                    f.write(f"Average Time / Iteration (s)   : {self.total_simulation_time / self.iter:.6f}\n")
    ...
    def save_results(self):
        options = ["Setup Visualisation", "Cavity Simulation", "Simulation Results", "Far-Field analysis", "Result Text","All"]
        ...
        if ok and choice:
            if choice == "All":
                ...
                if dir_path:
                    ...
                    self.save_result_text(os.path.join(dir_path, "05_results.txt"))
                    QMessageBox.information(self, "Saved", f"All figures saved in {dir_path}")

            elif choice == "Result Text":
                file_path, _ = QFileDialog.getSaveFileName(self, "Save Results", "Results.txt", "Text Files (*.txt)")
                if file_path:
                    self.save_result_text(file_path)
                    QMessageBox.information(self, "Saved", "Result text file saved.")
            else:
...
if __name__ == '__main__':
    print("=" * 60)
    print("FOX-LI LASER CAVITY SIMULATOR")
    print("Version 1")
    print("Engine: NumPy")
    print("=" * 60)
    app = QApplication(sys.argv)
    app.setWindowIcon(QIcon(resource_path("icons/app_icon_numpy.png")))
    window = FoxLiGUI()
    window.setWindowIcon(QIcon(resource_path("icons/app_icon_numpy.png")))
